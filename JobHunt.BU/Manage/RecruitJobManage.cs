...
namespace JobHunt.BU.Manage
{
    public class RecruitJobManage
    {
        ...
        public List<RecruitJobDTO> GetListFindJob()
        {
            ...
            var getLists = db.RecruitJobs.Where(x => x.RJStatus != (int)Common.Enum.EnumStatusJob.Active).OrderByDescending(x => x.RJCount).ToList();
        ...
        {
            db = new JobHuntEntities();
            var getEF = db.RecruitJobs.Where(x => x.RJType == type && x.RJExpirationDate >= DateTime.Now && x.RJStatus == (int?)Common.Enum.EnumStatusJob.Active).ToList();
            if (getEF.Count == 0)
                getEF = db.RecruitJobs.Where(x => x.RJExpirationDate >= DateTime.Now && x.RJStatus == (int?)Common.Enum.EnumStatusJob.Active).ToList();
            var listDTO = new List<RecruitJobDTO>();
            foreach (var cdd in getEF)
            {
                listDTO.Add(convertData.ConvertToDTO(cdd));
        ...
        public IEnumerable<RecruitJobDTO> GetListRecruitJobHaveSearchAndPaging(string keyWord, int? idCity, int? idProfesstion, int page, int pageSize)
        {
            ...
            {
                SearchList = db.RecruitJobs.Where(x => x.RJStatus == 2).ToList();
            }

            if (!string.IsNullOrEmpty(keyWord))
            {
                /*SearchList = SearchList.Where(x => x.RJTitle.Contains(keyWord) ||
                (x.Profession?.PFName ?? string.Empty).Contains(keyWord)).ToList();*/
                SearchList = SearchList.Where(x => x.RJTitle.Contains(keyWord) && x.RJStatus == 2).ToList();
            }

            if (idProfesstion != null)
                SearchList = SearchList.Where(x => x.RJ_ProfessionId == idProfesstion && x.RJStatus == 2).ToList();

            if (idCity != null)
                SearchList = SearchList.Where(x => x.RJCityId == idCity).ToList();

            SearchList = SearchList.Where(x => x.RJExpirationDate >= DateTime.Now && x.RJType != (int?)BU.Common.Enum.EnumStatusJob.Inactive && x.RJStatus == 2).ToList();
        ...
        public List<RecruitJobDTO> GetListRecentJobs(int WorkTypeId, int SalaryId, int? DistrictId)
        {
            var listDTO = new List<RecruitJobDTO>();
            db = new JobHuntEntities();
            var getListEF = db.RecruitJobs.Where(x => x.RJStatus == 2 && x.RJ_WorkTypeId == WorkTypeId && x.RJ_SalaryId == SalaryId && x.RJDistrictId == DistrictId && x.RJStatus != (int)Common.Enum.EnumStatusJob.Inactive && x.RJExpirationDate >= DateTime.Now).OrderByDescending(x => x.RJPostDate).ToList();
        ...
        public List<RecruitJobDTO> GetListRecruitJobsBySearch(string keyWord, int? status)
        {
            db = new JobHuntEntities();
            List<RecruitJobDTO> listRecruitJobDTO = new List<RecruitJobDTO>();
            var getLists = db.RecruitJobs.Where(x => x.RJStatus != (int)Common.Enum.EnumStatusJob.Active).OrderByDescending(x => x.RJPostDate).ToList();
