...
namespace JobHunt.Controllers
{
    public class RecruitJobController : Controller
    {
        ...
        static void SendEmail(object obj)
        {
            ...
            {
                SendMailToRegister(job.Path, j.Name, "tin đăng tuyển dụng", "https://localhost:44315/tuyen-dung/chi-tiet-viec-lam/" + BU.Common.GenerateUrl.GenerateSlug(job.job.RJTitle, job.job.RecruitJobId), j.Email);
            }
        }
        public static int SendMailToRegister(string path, string name, string tenht, string link, string mail)
        {
            ...
            try
            {
                ...
                var senderEmail = new MailAddress("websitedoan@gmail.com", "TechJobs");
                var receiverEmail = new MailAddress(mail);
                var password = "nljkgnluuxfegxnu";
                var sub = "Thông báo từ Tech Jobs";
                var body = content;
