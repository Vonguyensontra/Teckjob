...
namespace JobHunt.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(string type, LoginViewModel model, string returnUrl)
        {
            ...
            {
                return Json(new { message = "Sai định dạng. Hãy nhập lại!", status = "warn", isSuccess }, JsonRequestBehavior.AllowGet);
            }
            if (string.IsNullOrEmpty(type))
                return Json(new { message = "Hãy chọn người dùng bạn muốn đăng nhập!", status = "warn", isSuccess }, JsonRequestBehavior.AllowGet);
            // This doesn't count login failures towards account lockout
            // To enable password failures to trigger account lockout, change to shouldLockout: true
            var userFind = UserManager.FindByEmail(model.Email);
            if (userFind == null)
                return Json(new { message = "Email không tồn tại!", status = "error", isSuccess }, JsonRequestBehavior.AllowGet);
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        string messageResult = "", statusResult = "error", userId = "";
                        ...
                        if (UserManager.IsInRole(userFind.Id, nameRole))
                        {
                            ...
                            var candidate =  candidateManage.GetCandidateInfoByIdAspNetUser(loginUser.Id);
                            var recruit = recruitManage.GetRecruitInfoByIdAspNetUser(loginUser.Id);
                            if (role.Equals("candidate"))
                            {
                                ...
                                else if (candidate.CPStatus == 4)
                                {
                                    messageResult = "Tài khoản của bạn đã bị xóa!";
                                    statusResult = "error";
                                    AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                                }
                                else
                        ...
                        return Json(new { message = messageResult, status = statusResult, userId, isSuccess }, JsonRequestBehavior.AllowGet);
                ...
                default:
                    return Json(new { message = "Sai tên tài khoản hoặc mật khẩu!", status = "error", isSuccess }, JsonRequestBehavior.AllowGet);
            }
        }
